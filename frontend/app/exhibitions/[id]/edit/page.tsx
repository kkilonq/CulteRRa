...
export default function EditExhibitionPage() {
  ...
  useEffect(() => {
    if (!params?.id) return;
    fetch(`https://culterra-back-kkilonq.amvera.io/api/exhibition/${params.id}`)
      .then((res) => res.json())
      .then((data) => {
        setTitle(data.title);
    ...
    try {
      const response = await fetch(`https://culterra-back-kkilonq.amvera.io/api/exhibition/${params.id}`, {
        method: 'PATCH',
