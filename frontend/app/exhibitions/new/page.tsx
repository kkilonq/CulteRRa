  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errData.error || 'Ошибка при сохранении данных.');
      }
    } catch {
  ...
  return (
    <div className="space-y-6 max-w-md mx-auto">
      ...
      <div className="bg-white border border-stone-200 p-8 shadow-md">
        <h1 className="text-2xl font-light text-stone-900 font-['Cormorant_Garamond',serif] tracking-wide border-b border-stone-100 pb-3 mb-6">Регистрация выставки</h1>
        {error && <div className="text-xs text-red-700 bg-red-50 p-3 border border-red-100 mb-4">{error}</div>}
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="pt-2">
            <button type="submit" disabled={loading} className="w-full py-2 bg-[#8C2D19] hover:bg-[#5F1E10] text-white text-[10px] uppercase tracking-wider font-medium transition-colors disabled:opacity-50">
              {loading ? 'Сохранение...' : 'Внести в каталог'}
            </button>
          </div>
        </form>
      </div>
